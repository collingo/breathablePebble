...
static void window_load(Window *window) {
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "It's alive!");
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "bounds.size.w = %d, bounds.size.h = %d", bounds.size.w, bounds.size.h);
  ...
  // Trim bitmap layer and scroll content to fit image
  // GRect image_frame = layer_get_frame(image_layer);
  // bitmap_layer_set_size(image_layer, max_size);
  scroll_layer_set_content_size(scroll_layer, GSize(imagebounds.size.w, imagebounds.size.h));
